// Cascaded Shadow Maps (CSM) - https://sbcode.net/threejs/csm/
...
csmFolder.add(csm, "lightNear", 1, 1000, 1).onChange((event) => {
  for (var i = 0; i < csm.lights.length; i++) {
    csm.lights[i].shadow.camera.near = event;
    csm.lights[i].shadow.camera.updateProjectionMatrix();
  }
})
csmFolder.add(csm, "lightFar", 1, 1000, 1).onChange((event) => {
  for (var i = 0; i < csm.lights.length; i++) {
    csm.lights[i].shadow.camera.far = event;
    csm.lights[i].shadow.camera.updateProjectionMatrix();
  }
})
csmFolder.add(csm, "lightIntensity", .1, 2, .1).onChange((event) => {
  for (var i = 0; i < csm.lights.length; i++) {
    csm.lights[i].intensity = event
  }
})
csmFolder.open()
