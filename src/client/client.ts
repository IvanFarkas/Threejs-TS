// Constructive Solid Geometry - https://sbcode.net/threejs/csg/

import * as THREE from "three";
import {OrbitControls} from "three/examples/jsm/controls/OrbitControls";
import Stats from "three/examples/jsm/libs/stats.module";
import {CSGFromMesh, CSGToMesh} from "./utils/ConstructiveSolidGeometry";
...
});
{
  ...
  const cubeCSG = CSGFromMesh(cubeMesh);
  const sphereCSG = CSGFromMesh(sphereMesh);

  const cubeSphereIntersectCSG = cubeCSG.intersect(sphereCSG);
  const cubeSphereIntersectMesh = CSGToMesh(
  ...
  const cylinderCSG1 = CSGFromMesh(cylinderMesh1);
  const cylinderCSG2 = CSGFromMesh(cylinderMesh2);
  const cylinderCSG3 = CSGFromMesh(cylinderMesh3);

  const cylindersUnionCSG = cylinderCSG1.union(
    cylinderCSG2.union(cylinderCSG3),
  );
  const cylindersUnionMesh = CSGToMesh(cylindersUnionCSG, new THREE.Matrix4());
  ...
  const finalMesh = CSGToMesh(finalCSG, new THREE.Matrix4());
