...
// ## class CSG
// It was updated by belopot in 2020
...
function CSGFromPolygons(polygons) {
  let csg = new CSG();
  csg.polygons = polygons;
  return csg;
}

function CSGFromGeometry(geom, objectIndex) {
  let polys = [];
  if (geom.isGeometry) {
    let fs = geom.faces;
...
function CSGFromMesh(mesh, objectIndex) {
  let csg = CSGFromGeometry(mesh.geometry, objectIndex);
...
function CSGToMesh(csg, toMatrix, toMaterial) {
  let ps = csg.polygons;
  let geom;
  let g2;
...
export {CSGFromMesh, CSGToMesh, Plane};